        ...
        <div className="overflow-x-auto border-2 rounded-2xl">
          <table className="table">
            ...
            <tbody>
              <tr>
                <td>{t("cost")}</td>
                <td>{appointment?.clinic?.appointment_cost.toLocaleString() ?? 0} IQD</td>
              </tr>
              <tr>
                <td>{t("service")}</td>
              ...
              <tr>
                <td>{t("extraFees")}</td>
                <td>{Number(appointment?.extra_fees).toLocaleString() ?? 0} IQD</td>
              </tr>
              <tr>
                <td>{t("discount")}</td>
              ...
              {userType == "admin"
                ? appointment?.system_offers?.length != 0
                  ? appointment?.system_offers?.map(
                      (e: SystemOffers, index) => (
                        <tr key={index}>
                          <td>{t("systemOffer")} [{TranslateClient(e.title)}]</td>
                          <td>
