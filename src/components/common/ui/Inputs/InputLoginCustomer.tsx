  ...
  return (
    <div className={`${conClass} relative z-0`}>
      <input
        ...
        {...register(`${name}`)}
        id={name}
        className="block py-2.5 px-0 w-full   bg-transparent border-0 border-b-2 border-[#c1d5df] appearance-none  focus:outline-none focus:ring-0 focus:border-[#1FB8B9]  peer"
        placeholder=" "
      />
      <label
        htmlFor={name}
        className={
          `${labelClass} absolute   duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0` +
