...
const ExportButton = ({
  ...
  const handleExport = () => {
    const api = `${process.env.localApi}doctor/appointment-deductions/export${data.year && !data.month ? `?year=${data.year}` : data.month && !data.year ? `?month=${data.month}` : data.year && data.month ? `?year=${data.year}&month=${data.month}` : ""}`;
