...
const TableLogs = ({
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      return await AppointmentLogsService.make<AppointmentLogsService>().getAllAppointmentsLogs(
        appointment?.id ?? 0,
      );
    },
  });
          ...
          <tbody>
            {res?.map((e:AppointmentLogs, index) => (
              <tr key={index}>
                ...
                <td>{e.happen_in}</td>
                <td>
                  <Link href={`${appointment?.id}/tag/${e.id}`} className="btn btn-sm btn-square">
