...
(() => {
  ...
  async function getTokenInternal2(messaging2) {
    const pushSubscription = await getPushSubscription2(messaging2.swRegistration, messaging2.vapidKey);
    const subscriptionOptions = {
      vapidKey: messaging2.vapidKey,
      swScope: messaging2.swRegistration.scope,
      endpoint: pushSubscription.endpoint,
    ...
    const tokenDetails = await dbGet2(messaging2.firebaseDependencies);
    if (!tokenDetails) {
      return getNewToken2(messaging2.firebaseDependencies, subscriptionOptions);
    } else if (!isTokenValid2(tokenDetails.subscriptionOptions, subscriptionOptions)) {
      try {
        await requestDeleteToken2(messaging2.firebaseDependencies, tokenDetails.token);
      } catch (e) {
        console.warn(e);
      }
      return getNewToken2(messaging2.firebaseDependencies, subscriptionOptions);
    } else if (Date.now() >= tokenDetails.createTime + TOKEN_EXPIRATION_MS2) {
      return updateToken2(messaging2, {
        token: tokenDetails.token,
        createTime: Date.now(),
        subscriptionOptions
      });
  ...
  async function deleteTokenInternal2(messaging2) {
    const tokenDetails = await dbGet2(messaging2.firebaseDependencies);
    if (tokenDetails) {
      await requestDeleteToken2(messaging2.firebaseDependencies, tokenDetails.token);
      await dbRemove2(messaging2.firebaseDependencies);
    }
    const pushSubscription = await messaging2.swRegistration.pushManager.getSubscription();
    if (pushSubscription) {
      return pushSubscription.unsubscribe();
  ...
  async function updateToken2(messaging2, tokenDetails) {
    try {
      const updatedToken = await requestUpdateToken2(messaging2.firebaseDependencies, tokenDetails);
      const updatedTokenDetails = Object.assign(Object.assign({}, tokenDetails), { token: updatedToken, createTime: Date.now() });
      await dbSet2(messaging2.firebaseDependencies, updatedTokenDetails);
      return updatedToken;
    } catch (e) {
      throw e;
    }
  ...
  async function onSubChange(event, messaging2) {
    ...
    if (!newSubscription) {
      await deleteTokenInternal2(messaging2);
      return;
    }
    const tokenDetails = await dbGet2(messaging2.firebaseDependencies);
    await deleteTokenInternal2(messaging2);
    messaging2.vapidKey = (_b = (_a = tokenDetails === null || tokenDetails === void 0 ? void 0 : tokenDetails.subscriptionOptions) === null || _a === void 0 ? void 0 : _a.vapidKey) !== null && _b !== void 0 ? _b : DEFAULT_VAPID_KEY2;
    await getTokenInternal2(messaging2);
  }
  ...
  var SwMessagingFactory = (container) => {
    const messaging2 = new MessagingService2(container.getProvider("app").getImmediate(), container.getProvider("installations-internal").getImmediate(), container.getProvider("analytics-internal"));
  ...
  // src/workers/firebase-messaging-sw.ts
  var defaultConfig = {
    apiKey: true,
    projectId: true,
  ...
  if (messaging) {
    messaging.onBackgroundMessage((payload) => {
      ...
      self.addEventListener("notificationclick", (event) => {
        ...
        } else if (payload?.data?.type.includes(
