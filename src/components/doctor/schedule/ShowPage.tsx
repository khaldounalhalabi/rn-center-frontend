...
const ShowSchedulePage = ({days,gap}:{days:SchedulesDay,gap:number})=>{
    ...
    return (
        <PageCard>
            ...
            <div>
                <h1 className={'label text-xl font-semi bold w-fit'}>Appointment Gap : <span className={'mx-2 badge badge-success'}>{gap}</span></h1>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {daysArray().map((day, index) => (
                    <div key={index} className="bg-white p-4 border border-gray-200 rounded-lg shadow">
                        ...
                        {days[day.toLowerCase()] && days[day.toLowerCase()].length > 0 ? (
                            days[day.toLowerCase()].map((schedule) => (
                                <div  key={schedule.id} className="mb-4 p-2 border border-gray-300 rounded-md">
