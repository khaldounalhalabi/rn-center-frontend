...
const ChartDashboardAdmin = () => {
    ...
    const {data:earningsRes,isLoading,isFetching} = useQuery({
        queryKey: ['ChartAdmin'],
        queryFn: async () => {
            return await AppointmentDeductionsService.make<AppointmentDeductionsService>("admin").getEarningsByYear()
        }
    })

    const months = Array.from({ length: 12 }, (_, index) => dayjs().month(index).format("YYYY-MM"));

    const completedData = Array.isArray(earningsRes?.data) ? earningsRes.data : [];
            ...
            <div className="relative w-full h-400">
                ...
                <ResponsiveContainer width="100%" height={400}>
                    <AreaChart data={data}>
                        ...
                        />
                        <Tooltip />
                        <Area type="monotone" dataKey="completed" name={"Earnings"} stroke="#1fb8b9" fillOpacity={1} fill="url(#colorPv)" />
                        <defs>
