...
const Page = () => {
  ...
  const dataTableData: DataTableData<Clinic> = {
    createUrl: `/secretary/clinics/create`,
    schema: [
      {
        name: "id",
        label: `id`,
      ...
      },
      {
        ...
        render: (_undefined, clinic, setHidden) => (
          <ActionsButtons
            ...
            baseUrl={`/secretary/clinics`}
            setHidden={setHidden}
            deleteMessage={t("delete_warning")}
          >
            <Tooltip title={schedulesT("clinicSchedules")}>
              <Link href={`/secretary/clinics/schedules/${clinic?.id}`}>
    ...
    api: async (page, search, sortCol, sortDir, perPage, params) =>
      await ClinicsService.make(RoleEnum.SECRETARY).indexWithPagination(
        page,
        search,
        sortCol,
        sortDir,
