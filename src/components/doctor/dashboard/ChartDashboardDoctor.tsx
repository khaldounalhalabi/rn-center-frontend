...
const ChartDashboardDoctor = () => {
  const { data: groupedByMonth } = useQuery({
    ...
    queryFn: async () => {
      return await AppointmentService.make<AppointmentService>(
        "doctor",
      ).allGroupedByMonth(dayjs().format("YYYY"));
    },
  });
  ...
  const {
    ...
    queryFn: async () => {
      return await AppointmentService.make<AppointmentService>(
        "doctor",
      ).completedGroupedByMonthCopy(dayjs().format("YYYY"));
    },
  });
  ...
  const notCompletedData = Array.isArray(groupedByMonth?.data)
    ? groupedByMonth.data
    : [];
  const completedData = Array.isArray(completedByMonth?.data)
    ? completedByMonth.data
    : [];

  const data = months.map((month) => ({
    date: dayjs(month).format("MMM"),
    notCompleted:
      notCompletedData.find((item) => item.date === month)?.appointment_count ||
      0,
    completed:
      completedData.find((item) => item.date === month)?.appointment_count || 0,
  }));
