...
const ChangeAllStatusSelector = ({ids,closeModalStatus}:{ids:number[],closeModalStatus:any})=>{
    ...
    const handleSelect =async (data:any)=>{
      ...
      return await AppointmentDeductionsService.make<AppointmentDeductionsService>("admin").bulkToggleStatus(dataSend).then((res)=>{
          revalidate()
          closeModalStatus()
          return res
      })
    ...
    return (
       <Form handleSubmit={handleSelect}>
           ...
           <SelectPopOverFrom status={""} ArraySelect={AppointmentDeductionsStatusArray()} name={'status'} handleSelect={()=>undefined}/>
       </Form>
    )
}
