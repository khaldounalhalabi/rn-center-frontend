...
const ExportButton = ({data,close}:{data:{year:string,month:string},close:any}) => {
    ...
    const handleExport = () => {
        const api = `${process.env.localApi}doctor/clinic-transactions/export${data.year && !data.month?`?year=${data.year}`:data.month && !data.year?`?month=${data.month}`:data.year && data.month?`?year=${data.year}&month=${data.month}`:""}`
