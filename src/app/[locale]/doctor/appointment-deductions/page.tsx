...
const Page = () => {
    ...
    const tableData: DataTableData<AppointmentDeductions> = {
        ...
        api: async (page, search, sortCol, sortDir, perPage, params) =>
            await AppointmentDeductionsService.make<AppointmentDeductionsService>("doctor").indexWithPagination(
                page,
                search,
                sortCol,
                sortDir,
            ...
            <PageCard>
                ...
                <Grid md={2}>
                    <label className="label">
                        Subscription Start :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.subscription_start}</span>
                    </label>
                    <label className="label">
                        Total Cost :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.total_cost}</span>
                    </label>

                    <label className="label">
                        Subscription End :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.subscription_end}</span>
                    </label>
                    <label className="label">
                        Subscription Cost :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.subscription_cost}</span>
                    </label>
                    <label className="label">
                        Clinic Balance :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.clinic_balance}</span>
                    </label>

                    <label className="label">
                        Appointments Deductions :
                        <span className="bg-base-200 px-2 rounded-xl text-lg">{balance?.data?.appointments_deductions}</span>
                    </label>
                </Grid>
            </PageCard>
            <DataTable {...tableData} />
