        ...
        <div>
            <div>
                <h1 className={"label text-xl font-semi bold w-fit"}>
                    {("Appointment Gap")} :{" "}
                    {/*// @ts-ignore*/}
                    <span className={"mx-2 badge badge-success"}>{clinic?.schedules?.appointment_gap}</span>
                </h1>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {daysArray().map((day, index) => (
                    ...
                    >
                        ...
                        {clinic?.schedules?.[day.toLowerCase()] && clinic?.schedules[day.toLowerCase()].length > 0 ? (
