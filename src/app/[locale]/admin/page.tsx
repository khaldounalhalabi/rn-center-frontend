...
const Page = () => {
    ...
    const {data,isLoading,isFetching} = useQuery({
        ...
        queryFn:async ()=>{
            return await StatisticService.make<StatisticService>("admin").adminIndexPageStatistics()
        }
    })
    ...
    <>
      <div className={"flex flex-col justify-between md:flex-row my-6"}>
        <div className="md:w-[40%] w-full px-4">
          <UserDataAdmin statisticsRes={statisticsRes} isLoading={isLoading} isFetching={isFetching}/>
        </div>

        <div className={" md:max-w-[58%] pr-4 w-full"}>
          <CardsAppointmentAdmin statisticsRes={statisticsRes} isLoading={isLoading} isFetching={isFetching}/>
          <Card>
